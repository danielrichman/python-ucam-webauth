...
class TestRSA(object):
    def test_good_signatures(self):
        for key_name, key_pkcs1 in key_pkcs1s.iteritems():
            key = load_key(key_pkcs1)

            for data_name, digest in digests.iteritems():
    ...
    def test_bad_signatures(self):
        for key_name, key_pkcs1 in key_pkcs1s.iteritems():
            ...
            for data_name, digest in digests.iteritems():
                ...
                for signature_name, signature in signatures.iteritems():
    ...
    def test_rejects_garbage_key(self):
        ...
        if sys.version_info[0] >= 3:
            assert_raises(TypeError, load_key, u"asdfasdf")
    ...
    def test_rejects_garbage_digest(self):
        ...
        if sys.version_info[0] >= 3:
            assert_raises(TypeError, key.verify, u"short digest", signature)
    ...
    def test_rejects_garbage_signature(self):
        ...
        if sys.version_info[0] >= 3:
            assert_raises(TypeError, key.verify, digest, u"asdf")

    def test_rejects_bad_length_digest(self):
        key = load_key(key_pkcs1s["a"])
        signature = signatures["1a"]
        assert_raises(ValueError, key.verify, "short digest", signature)
